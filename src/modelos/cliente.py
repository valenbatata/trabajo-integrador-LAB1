class Cliente:
    ...
    def __init__(self, nombre: str, apellido: str, email: str, password: str, dni: str, telefono: str, ciudad: str, pais: str):
        # En lugar de asignar directamente, usamos los setters para validar
    ...
    def set_nombre(self, nombre: str):
        if not isinstance(nombre, str) or not nombre.strip().isalpha():
            raise ValueError("El nombre debe contener solo letras.")
        self.__nombre = nombre.strip().capitalize()

    def set_apellido(self, apellido: str):
        if not isinstance(apellido, str) or not apellido.strip().isalpha():
    ...
    def set_dni(self, dni: str):
        # Intento convertir a string y validar dígitos
        try:
    ...
    def set_telefono(self, telefono: str):
        ...
        if not telefono_str.isdigit() or len(telefono_str) < 7:
            raise ValueError("El teléfono debe contener solo números válidos.")
        self.__telefono = telefono_str

    def set_ciudad(self, ciudad: str):
        if not isinstance(ciudad, str) or not ciudad.strip().isalpha():
            raise ValueError("La ciudad debe contener solo letras.")
        self.__ciudad = ciudad.strip().capitalize()

    def set_pais(self, pais: str):
        if not isinstance(pais, str) or not pais.strip().isalpha():
            raise ValueError("El país debe contener solo letras.")
